...
```{r expression_matrix_check, echo=TRUE}
...
checkExpMat <- function( expMat1, sampleVec1, expMat2,  sampleVec2 ){
    ...
    if( is.null( rownames(expMat1)) | is.null(rownames(expMat2)) )
      print("Error: no rownames for expMat1 or expMat2")
    ...
    if( length(sampleVec1) != ncol(expMat1) )
      print("Error: conflict between expMat1 and sampleVec")
    if( length(sampleVec2) != ncol(expMat2) )
      print("Error: conflict between expMat2 and sampleVec")
    
    if( length( which( (rownames(expMat1) == rownames(expMat2)) == FALSE )) > 0 ) 
      print("Error:conflicted gene order in expMat1 and expMat2")
  
    sample.un1 <- unique(sampleVec1)
    sample.un2 <- unique(sampleVec2)
    if( length(sample.un1) != length(sample.un2) )
      print("Error: different time_points tissues unique samples between expMat1 and expMat2")
    
    if( length( which( (rownames(sample.un1) == rownames(sample.un2)) == FALSE ) ) > 0 )
      print("Error: conflict time_points tissues unique samples between expMat1 and expMat2")
...
```{r expression_matrix_mean, echo=TRUE}
...
feature_mat_6 <- as.data.frame(feature_mat_6)
feature_matrix$cv6 <- feature_mat_6$cv2
feature_matrix$zScore6_1.1 <- feature_mat_6$zScore2_1.1
feature_matrix$zScore6_1.2 <- feature_mat_6$zScore2_1.2
feature_matrix$expression6_1.1 <- feature_mat_6$expression2_1.1
feature_matrix$expression6_1.2 <- feature_mat_6$expression2_1.2
feature_matrix$coef20 <- lateral_root_data$Coef.Col_NAA_2h.Col_NPA_0h_pw_LRIvanneste
feature_matrix$coef62 <- lateral_root_data$Coef.Col_NAA_6h.Col_NAA_2h_pw_LRIvanneste
feature_matrix$coef60 <- lateral_root_data$Coef.Col_NAA_6h.Col_NPA_0h_pw_LRIvanneste

feature_matrix$pval20 <- lateral_root_data$p.value.Col_NAA_2h.Col_NPA_0h_pw_LRIvanneste
feature_matrix$pval62 <- lateral_root_data$p.value.Col_NAA_6h.Col_NAA_2h_pw_LRIvanneste
feature_matrix$pval60 <- lateral_root_data$ p.value.Col_NAA_6h.Col_NPA_0h_pw_LRIvanneste
...
chloroplast_rows <- rownames(featureMat[rownames(featureMat) %like% "ATC", ] )

negatives <- c(ribosomal, mitochondria, chloroplast, mitochondria_rows, chloroplast_rows)
negatives <- unique(negatives)
negatives <- as.data.frame(negatives)
positives <- as.data.frame(positives)

###OSMOTIC STRESS GENES
osmotic_stress_genes <- read.table("~/Google Drive/My Drive/Dissertation- G drive/Arabi/sucrose_in_lateral_root/osmotic_stress_genes.txt", header = TRUE)
rownames(osmotic_stress_genes) <- osmotic_stress_genes$Gene_stable_ID
neg3 <- intersect(osmotic_stress_genes$Gene_stable_ID, rownames(featureMat))
neg <- neg3
neg <- unique(neg)
...
positiveSamples_matrix <- feature_matrix[positiveSamples, ]
negativeSamples_matrix <- feature_matrix[negativeSamples, ]
dists_pos <- pdist(positiveSamples_matrix, negativeSamples_matrix)
distance_matrix <- as.matrix(dists_pos)
rownames(distance_matrix) <- rownames(positiveSamples_matrix)
colnames(distance_matrix) <- rownames(negativeSamples_matrix)
distance_matrix_df <- as.data.frame(distance_matrix)
df_final <- data.frame(matrix(0, nrow = 123, ncol=1))
for (x in negativeSamples) {
  rankv= "Rank"
  rank_var_new=paste(rankv, x, sep="_" )
  print(rank_var_new)
  rank_var_newx <- apply(distance_matrix_df[x], 2, rank)
...
df_final_test <- subset(df_final, select = -matrix.0..nrow...123..ncol...1.)
head(df_final_test)
average_rank <- rowMeans(df_final_test)
df_final_test <- cbind(df_final_test, average_rank)
rownames(df_final_test) <- rownames(positiveSamples_matrix)
data_rank <- df_final_test[order(-average_rank),]
number_negative_set <- nrow(as.data.frame(negativeSamples)) 
rows_max_ed <- rownames(head(data_rank, number_negative_set)) 
positiveSamples <- rows_max_ed
